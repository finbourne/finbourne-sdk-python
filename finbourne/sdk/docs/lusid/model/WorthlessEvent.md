...
| **announcement_date** | **datetime** | Optional | The date the issuer or agent announces the write-off. Optional — null when no separate announcement date is provided.  When populated, must be &lt;&#x3D; RecordDate. |
...
```python
from finbourne.sdk.services.lusid.models.WorthlessEvent import WorthlessEvent

instance = WorthlessEvent(
    ...
    announcement_date=datetime.now(),  # optional — The date the issuer or agent announces the write-off. Optional — null when no separate announcement date is provided.  When populated, must be &lt;&#x3D; RecordDate.
