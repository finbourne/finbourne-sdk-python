...
| **strike** | **float** | Optional | Agreed forward price at trade inception. Reference only — not used in the market value calculation,  which consumes the pre-netted price from the quote store. |
| **delivery_type** | **str** | Required | Whether the forward settles in cash or through physical delivery of the underlying.                Supported string (enumeration) values are: [Cash, Physical]. Available values: Cash, Physical. |
...
```python
from finbourne.sdk.services.lusid.models.CommodityForward import CommodityForward

instance = CommodityForward(
    ...
    strike=0.0,  # optional — Agreed forward price at trade inception. Reference only — not used in the market value calculation,  which consumes the pre-netted price from the quote store.
    delivery_type="...",  # required — Whether the forward settles in cash or through physical delivery of the underlying.                Supported string (enumeration) values are: [Cash, Physical]. Available values: Cash, Physical.
