...
| **lapse_elections** | [List[LapseElection]](LapseElection.md) | Optional | List of possible lapse elections for this merger event (NOAC). |
| **unknown_proceeds_elections** | [List[UnknownProceedsElection]](UnknownProceedsElection.md) | Optional | List of possible unknown-proceeds elections for this merger event (UNKNOWN) — the outturn is not yet known. |
...
instance = MergerEvent(
    ...
    cash_offer_elections=[],  # optional — List of possible CashOfferElections for this merger event. Under ParticipationType.Mandatory  a single CashOfferElection may be the event&#39;s only election — a cash-only merger — in which  case no NewInstrument is required. Under ParticipationType.MandatoryWithChoices cash offers  are provided alongside the other elections on the event.
    ...
    lapse_elections=[],  # optional — List of possible lapse elections for this merger event (NOAC).
    unknown_proceeds_elections=[],  # optional — List of possible unknown-proceeds elections for this merger event (UNKNOWN) — the outturn is not yet known.
