...
|------|------|----------|-------------|
| **start_date** | **datetime** | Required | The settlement date of the FRA |
...
```python
from finbourne.sdk.services.lusid.models.ForwardRateAgreement import ForwardRateAgreement

instance = ForwardRateAgreement(
    start_date=datetime.now(),  # required — The settlement date of the FRA
    ...
    fixing_date=datetime.now(),  # required — The date at which the rate to be paid, the reference rate, is confirmed/observed.
    fra_rate=0.0,  # required — The rate at which the FRA is traded.
    notional=0.0,  # required — The amount for which the FRA is traded.
