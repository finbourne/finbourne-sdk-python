...
| **announcement_date** | **datetime** | Optional | Date on which the dividend is announced by the company. |
...
```python
from finbourne.sdk.services.lusid.models.CashDividendEvent import CashDividendEvent

instance = CashDividendEvent(
    payment_date=datetime.now(),  # optional — The date the company begins distributing the dividend.
    ex_date=datetime.now(),  # optional — The first business day on which the dividend is not owed to the buying party.
    cash_elections=[],  # required — Possible elections for this event, each keyed with a unique identifier.
    announcement_date=datetime.now(),  # optional — Date on which the dividend is announced by the company.
