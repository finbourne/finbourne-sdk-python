...
class PropertyDefinitionSearchResult(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["href", "key", "valueType", "displayName", "dataTypeId", "type", "unitSchema", "domain", "scope", "code", "valueRequired", "lifeTime", "constraintStyle", "propertyDefinitionType", "propertyDescription", "derivationFormula", "isFilterable", "links"]
    ...
    def type_validate_enum(cls, value):
        ...
        _allowed = ['Label', 'Metric', 'Information']
        if len(_allowed) != 1:
            return value
        if value not in _allowed:
            raise ValueError(f"must be one of enum values {_allowed}")
    ...
    def unit_schema_validate_enum(cls, value):
        ...
        _allowed = ['NoUnits', 'Basic', 'Iso4217Currency']
        if len(_allowed) != 1:
            return value
        if value not in _allowed:
            raise ValueError(f"must be one of enum values {_allowed}")
    ...
    @field_validator('property_definition_type')
    def property_definition_type_validate_enum(cls, value):
        ...
        _allowed = ['ValueProperty', 'DerivedDefinition']
        if len(_allowed) != 1:
            return value
        if value not in _allowed:
            raise ValueError(f"must be one of enum values {_allowed}")
    ...
    def from_dict(cls, obj: dict) -> PropertyDefinitionSearchResult:
        ...
        if not isinstance(obj, dict):
            return PropertyDefinitionSearchResult.model_validate(obj)

        _obj = PropertyDefinitionSearchResult.model_validate({
            "href": obj.get("href"),
            "key": obj.get("key"),
...
PropertyDefinitionSearchResult.model_rebuild()
