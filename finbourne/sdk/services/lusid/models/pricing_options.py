...
class PricingOptions(BaseModel):
    ...
    risk_engine:  Optional[StrictStr] = Field(default=None,alias="riskEngine", description="Which engine computes first-order Risk/* measures. One of \"Bump\" (default: central  finite differences by bump-and-revalue - the historical behaviour, used when this is  absent), \"Parity\" (bump computes and is reported, the adjoint engine independently checks  every calculator and any disagreement fails the measure loudly - the recommended  enablement gate), or \"Adjoint\" (algorithmic differentiation where an evaluator exists,  with silent fallback to Bump elsewhere - selecting it can never reduce coverage). Available values: Bump, Adjoint, Parity.") 
    ...
    def from_dict(cls, obj: dict) -> PricingOptions:
        ...
        if not isinstance(obj, dict):
            return PricingOptions.model_validate(obj)

        _obj = PricingOptions.model_validate({
            "model_selection": ModelSelection.from_dict(_v) if (_v := obj.get("modelSelection")) is not None else None,
            ...
            "return_zero_pv": ReturnZeroPvOptions.from_dict(_v) if (_v := obj.get("returnZeroPv")) is not None else None,
