...
class VolatilitySwap(LusidInstrument):
    """
    ...
    dom_ccy:  StrictStr = Field(...,alias="domCcy", description="The domestic currency of the instrument, in which the looked-up price and market value are  denominated. Quotes supplied in a minor unit of this currency (e.g. GBp) are re-denominated  to it by the lookup pricer.") 
    strike: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="The variance or volatility strike agreed at trade date, stored for reference only.  Not used in valuation or close-out.")
    notional: Union[StrictFloat, StrictInt] = Field(description="The agreed notional for the swap. The sign conveys direction (a negative notional held long  produces a negative market value).")
    price_denominator: StrictInt = Field(description="Scalar divisor applied in the market value calculation:  MktVal = Quantity x Notional x Price / PriceDenominator.  1 for volatility swaps (VOLS) and 100 for variance swaps (VARS). Must be positive.", alias="priceDenominator")
    time_zone_conventions: Optional[TimeZoneConventions] = Field(default=None, alias="timeZoneConventions")
    underlying:  Optional[StrictStr] = Field(default=None,alias="underlying", description="Free-text reference label identifying the underlying index or asset (e.g. 'SPX', 'SX5E', 'KOSPI2').  Reference only; not used in valuation.") 
    ...
    __properties: ClassVar[List[str]] = ["instrumentType", "startDate", "maturityDate", "domCcy", "strike", "notional", "priceDenominator", "timeZoneConventions", "underlying"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VolatilitySwap:
        """Create an instance of VolatilitySwap from a JSON string"""
    ...
    def to_dict(self, by_alias=True):
        ...
        # set to None if underlying (nullable) is None
        # and model_fields_set contains the field
        if self.underlying is None and "underlying" in self.model_fields_set:
            _dict['underlying'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> VolatilitySwap:
        """Create an instance of VolatilitySwap from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return VolatilitySwap.model_validate(obj)

        _obj = VolatilitySwap.model_validate({
            ...
            "price_denominator": obj.get("priceDenominator"),
