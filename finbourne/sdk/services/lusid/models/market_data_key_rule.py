...
class MarketDataKeyRule(BaseModel):
    ...
    price_source:  Optional[StrictStr] = Field(default=None,alias="priceSource", description="The source of the quote. For a given provider/supplier of market data there may be an additional qualifier, e.g. the exchange or bank that provided the quote") 
    mask:  Optional[StrictStr] = Field(default=None,alias="mask", description="Allows for partial or complete override of the market asset resolved for a dependency  Either a named override or a dot separated string (A.B.C.D.*).  e.g. for Rates curve 'EUR.*' will replace the resolve MarketAsset 'GBP/12M', 'GBP/3M' with the EUR equivalent, if there  are no wildcards in the mask, the mask is taken as the MarketAsset for any dependency matching the rule.") 
    source_system:  Optional[StrictStr] = Field(default=None,alias="sourceSystem", description="If set, this parameter will seek an external source of market data.  Optional and, if omitted, will default to \"Lusid\".  This means that data will be retrieved from the LUSID Quote Store and LUSID Complex Market Data Store.                This can be set to \"MarketDataOverrides\" if Supplier is set to \"Client\".") 
    ...
    __properties: ClassVar[List[str]] = ["key", "supplier", "dataScope", "quoteType", "field", "quoteInterval", "asAt", "priceSource", "mask", "sourceSystem", "fallThroughOnAccessDenied"]
    ...
    def quote_type_validate_enum(cls, value):
        ...
        _allowed = ['Price', 'Spread', 'Rate', 'LogNormalVol', 'NormalVol', 'ParSpread', 'IsdaSpread', 'Upfront', 'Index', 'Ratio', 'Delta', 'PoolFactor', 'InflationAssumption', 'DirtyPrice', 'PrincipalWriteOff', 'InterestDeferred', 'InterestShortfall', 'ConstituentWeightFactor']
        if len(_allowed) != 1:
            return value
    ...
    def from_dict(cls, obj: dict) -> MarketDataKeyRule:
        ...
        if not isinstance(obj, dict):
            return MarketDataKeyRule.model_validate(obj)

        _obj = MarketDataKeyRule.model_validate({
            "key": obj.get("key"),
...
MarketDataKeyRule.model_rebuild()
