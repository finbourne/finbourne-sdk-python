...
class UpsertReferencePortfolioConstituentsRequest(BaseModel):
    ...
    def weight_type_validate_enum(cls, value):
        ...
        _allowed = ['Static', 'Floating', 'Periodical']
        if len(_allowed) != 1:
            return value
    ...
    def period_type_validate_enum(cls, value):
        ...
        _allowed = ['Daily', 'Weekly', 'Monthly', 'Quarterly', 'Annually']
        if len(_allowed) != 1:
            return value
        if value not in _allowed:
            raise ValueError(f"must be one of enum values {_allowed}")
    ...
    def to_dict(self, by_alias=True):
        ...
        # and model_fields_set contains the field
        if self.period_count is None and "period_count" in self.model_fields_set:
    ...
    def from_dict(cls, obj: dict) -> UpsertReferencePortfolioConstituentsRequest:
        ...
        if not isinstance(obj, dict):
            return UpsertReferencePortfolioConstituentsRequest.model_validate(obj)

        _obj = UpsertReferencePortfolioConstituentsRequest.model_validate({
...
UpsertReferencePortfolioConstituentsRequest.model_rebuild()
