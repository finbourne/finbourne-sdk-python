...
class InflationLinkedBond(LusidInstrument):
    ...
    calculation_type:  Optional[StrictStr] = Field(default=None,alias="calculationType", description="The calculation type applied to the bond coupon and principal amount.  The default CalculationType is `Standard`.    Supported string (enumeration) values are: [Standard, Quarterly, Ratio, Brazil, StandardAccruedOnly, RatioAccruedOnly, StandardWithCappedAccruedInterest].") 
    ...
    def to_dict(self, by_alias=True):
        ...
        if self.inflation_index_conventions:
            _dict['inflationIndexConventions'] = self.inflation_index_conventions.to_dict(by_alias=by_alias)
        ...
        # override the default output from pydantic by calling `to_dict()` of amortisation_schedule
        if self.amortisation_schedule:
            _dict['amortisationSchedule'] = self.amortisation_schedule.to_dict(by_alias=by_alias)
        ...
        # and model_fields_set contains the field
        if self.base_cpi is None and "base_cpi" in self.model_fields_set:
            _dict['baseCPI'] = None

        # set to None if base_cpi_date (nullable) is None
        # and model_fields_set contains the field
        if self.base_cpi_date is None and "base_cpi_date" in self.model_fields_set:
    ...
    def from_dict(cls, obj: dict) -> InflationLinkedBond:
        ...
        if not isinstance(obj, dict):
            return InflationLinkedBond.model_validate(obj)

        _obj = InflationLinkedBond.model_validate({
            ...
            "amortisation_schedule": StepSchedule.from_dict(_v) if (_v := obj.get("amortisationSchedule")) is not None else None
        })
...
InflationLinkedBond.model_rebuild()
