...
class TransactionConfigurationMovementData(BaseModel):
    ...
    def movement_types_validate_enum(cls, value):
        ...
        _allowed = ['Settlement', 'Traded', 'StockMovement', 'FutureCash', 'Commitment', 'Receivable', 'CashSettlement', 'CashForward', 'CashCommitment', 'CashReceivable', 'Accrual', 'CashAccrual', 'ForwardFx', 'CashFxForward', 'Carry', 'CarryAsPnl', 'VariationMargin', 'Capital', 'Fee', 'LimitAdjustment', 'BalanceAdjustment', 'Deferred', 'CashDeferred']
        if len(_allowed) != 1:
            return value
    ...
    def from_dict(cls, obj: dict) -> TransactionConfigurationMovementData:
        ...
        if not isinstance(obj, dict):
            return TransactionConfigurationMovementData.model_validate(obj)

        _obj = TransactionConfigurationMovementData.model_validate({
            ...
            "mappings": [TransactionPropertyMapping.from_dict(_item) for _item in _v] if (_v := obj.get("mappings")) is not None else None,
            "name": obj.get("name"),
            "movement_options": obj.get("movementOptions")
        })
        return _obj

TransactionConfigurationMovementData.model_rebuild()
