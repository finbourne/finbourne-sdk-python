...
class FileAccessToken(collections.UserString):
    ...
    def data(self) -> str:  # type: ignore[override]
        ...
        str
            Access token
        """
        if self.__token_data["expires"] <= datetime.datetime.now(tz=datetime.timezone.utc):
            try:
                ...
                self.__token_data["expires"] = (
                    datetime.datetime.now(tz=datetime.timezone.utc) + datetime.timedelta(seconds=self.__expiry_time)
                )
            except OSError:
